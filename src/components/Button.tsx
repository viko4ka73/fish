const Button = ({ label, href, Cart, Order, Catalog, Enter }: any) => {
    ...
    return (
        <button className={buttonClass}>
            <a href={href} className={Cart? 'text-dark-blue font-bold text-2xl max-2xl:text-xl  leading-normal'  : Order ? 'max-2xl:text-xl max-[1900px]:text-2xl  font-normal  text-3xl leading-none' : Catalog ? 'text-4xl text-white-400 font-bold max-2xl:text-3xl' : Enter ? 'text-2xl text-white font-bold': ''}>
                {label}
            </a>
        </button>
    );
